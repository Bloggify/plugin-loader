"use strict";

const fs = require("fs")
    , BloggifyPlugin = require("bloggify-plugin-class")
    , oneByOne = require("one-by-one")
    , bindy = require("bindy")
    , noop = require("noop6")
    , typpy = require("typpy")
    ;

module.exports = class BloggifyPluginLoader {

    /**
     * BloggifyPluginLoader
     * The Bloggify plugin loader.
     *
     * @name bloggifyPluginLoader
     * @function
     * @param {BloggifyCore} bloggify The `BloggifyCore` instance.
     * @return {BloggifyPluginLoader} The `BloggifyPluginLoader` instance.
     */
    constructor (bloggify) {
        this.bloggify = bloggify;
        this.plugins = {};
    }

    /**
     * getPluginPaths - Returns the plugin's path.
     *
     * @param  {Object} c The called plugin.
     * @return {String} The path of the plugin.
     */
    getPluginPaths (c) {
        return {
            pluginPath: this.bloggify.paths.pluginPath(c)
          , name: c
        };
    }

    /**
     * namesToPaths - Returns the path list for each plugin.
     *
     * @param  {Array} names The list of plugin names.
     * @return {Array} The path list.
     */
    namesToPaths (names) {
        return names.map(c => this.getPluginPaths(c));
    }

    /**
     * listPluginDirs -
     *
     * @param  {Array} names The list of plugin names.
     * @param  {Function} cb The callback function.
     * @return {Array}       The plugin paths.
     */
    listPluginDirs (names, cb) {
        if (typeof names === "function") {
            cb = names;
            names = null;
        }
        cb = cb || noop;
        if (Array.isArray(names)) {
            return cb(null, this.namesToPaths(names));
        }
        let pluginsDir = this.bloggify.paths.plugins;
        fs.readdir(pluginsDir, (err, dirs) => {
            if (err) {
                if (err.code === "ENOENT") {
                    return cb(null, [], err);
                }
                return cb(err, []);
            }
            cb(null, this.namesToPaths(dirs.sort()));
        });
    }

    /**
     * getPlugin - Returns called plugin's content if it's valid. Otherwise, it returns a new one.
     *
     * @param  {Object} plugin The called plugin.
     * @return {Object}        The verified plugin.
     */
    getPlugin (plugin) {

        if (typpy(plugin, BloggifyPlugin)) {
            return plugin;
        }

        if (typeof plugin === "string") {
            return this.getPlugin(this.getPluginPaths(plugin));
        }

        return new BloggifyPlugin(
            plugin.name
          , plugin.pluginPath
          , this.bloggify
        );
    }

    /**
     * listPlugins - Returns the plugins list.
     *
     * @param  {Array} names The list of plugin names.
     * @param  {Function} cb The callback function.
     * @return {Array}       Plugins list.
     */
    listPlugins (names, cb) {
        if (!cb) {
            cb = names;
            names = null;
        }
        cb = cb || noop;
        this.listPluginDirs(names, (err, dirs, warning) => {
            let plugs = dirs.map(c => this.getPlugin(c));
            cb(err, plugs, warning);
        });
    }

    /**
     * initPlugin -
     *
     * @param  {Object} plug description
     * @param  {Function} cb   The callback function.
     * @return {Object}      description
     */
    initPlugin (plug, cb) {
        if (this.plugins[plug.name]) {
            return cb(null, this.plugins[plug.name]);
        }
        this.plugins[plug.name] = plug;
        plug.init(cb);
    }

    /**
     * loadAll - description
     *
     * @param  {Array} names The list of plugin names.
     * @param  {Function} cb    The callback function.
     * @return {type}       description
     */
    loadAll (names, cb) {
        if (typeof names === "function") {
            cb = names;
            names = null;
        }
        cb = cb || noop;
        if (!names) {
            this.bloggify.log("A list of plugins was not provided, therefore all the plugins in the plugin directory will be loaded.");
        }
        this.listPlugins(names, (err, plugins) => {
            if (err) { return cb(err); }
            oneByOne(bindy(plugins, (c, next) => this.initPlugin(c, next)), cb);
        });
    }

    /**
     * loadPlugin - 
     *
     * @param  {Object} plugin description
     * @param  {Function} cb     The callback function.
     * @return {Object}        description
     */
    loadPlugin (plugin, cb) {
        plugin = this.getPlugin(plugin);
        this.initPlugin(plugin, cb);
    }

    /**
     * get -
     *
     * @param  {String} name description
     * @param  {type} mod  description
     * @return {Object}      description
     */
    get (name, mod) {
        let plug = this.plugins[name]
                 || this.plugins["bloggify-" + name]
                 || this.plugins["_bloggify-" + name]
                 || this.plugins[name.replace(/^_?bloggify\-/g, "")]
                 ;

        if (mod) {
            return plug._module;
        }
        return plug;
    }
};
